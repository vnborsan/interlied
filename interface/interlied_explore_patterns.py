"""
This function explores patterns and complementary lyric tokens.

*Input: dataframe obtained by interlied_pattern_analysis function.
...
def patterns_lyric():
    ...
    root.configure(bg='#94EA64')
    root.title("Explore Patterns")
    ...
    label_q1 = ttk.Label(q_frame, text="Insert Pattern")
    ...
    def Load_excel_data():
        ...
        for row in df_rows:
            if (interval_q == row[1]) and (dur_q in str(row[3])) and (title_q in str(row[6])) and (composer_q in str(row[7])) and (key_q in str(row[10])) and (token_q in str(row[15])):
